...
import { variable } from './digital.assets.variable';
import { repository } from './digital.assets.repository';
import { transformer } from './digital.assets.transformer';
import {
  NOT_FOUND,
  ROLE_ADMIN,
  ROLE_AGENT,
...
export default class Controller {
  public async list(req: Request, res: Response) {
    try {
      ...
      if (clientId != undefined)
        condition = { digital_assets_holder: clientId };
      else if (![ROLE_ADMIN, ROLE_AGENT].includes(role_name))
        condition = { digital_assets_holder: client_id };

      const result = await repository.list(condition);
      if (result?.length < 1)
        return response.success(NOT_FOUND, null, res, false);
      const digitalAssets = await transformer.list(result);
      return response.success(SUCCESS_RETRIEVED, digitalAssets, res);
    } catch (err: any) {
      return helper.catchError(
        `digital assets all-data: ${err?.message}`,
        500,
        res
      );
    }
  ...
  public async index(req: Request, res: Response) {
    try {
      ...
      if (rows?.length < 1)
        return response.success(NOT_FOUND, null, res, false);
      const digitalAssets = await transformer.list(rows);
      return response.success(
        SUCCESS_RETRIEVED,
        { total: count, values: digitalAssets },
        res
      );
    } catch (err: any) {
      return helper.catchError(
        `digital assets index: ${err?.message}`,
        500,
        res
      );
    }
  ...
  public async detail(req: Request, res: Response) {
    try {
      ...
      const digitalAssets = await transformer.detail(result);
      return response.success(SUCCESS_RETRIEVED, digitalAssets, res);
    } catch (err: any) {
      return helper.catchError(
        `digital assets detail: ${err?.message}`,
        500,
        res
      );
    }
  ...
  public async create(req: Request, res: Response) {
    try {
      ...
      return helper.catchError(
        `digital assets create: ${err?.message}`,
        500,
        res
      );
    }
  ...
  public async update(req: Request, res: Response) {
    try {
      const id: string = req?.params?.id || '';
      const check = await repository.detail({ digital_assets_id: id });
      ...
      await repository.update({
        ...
        condition: { digital_assets_id: id },
      });
      return response.success(SUCCESS_UPDATED, null, res);
    } catch (err: any) {
      return helper.catchError(
        `digital assets update: ${err?.message}`,
        500,
        res
      );
    }
  ...
  public async delete(req: Request, res: Response) {
    try {
      ...
      return helper.catchError(
        `digital assets delete: ${err?.message}`,
        500,
        res
      );
    }
  ...
  public async export(req: Request, res: Response) {
    ...
    try {
      ...
      return helper.catchError(
        `digital assets ${type}: ${err?.message}`,
        500,
        res
      );
    }
...
export const digitalassets = new Controller();
